...
def main():
    ...
    try:
        ...
        vehicle2_location = vehicle2_vehicle.get_transform()
        print(vehicle2_location)
        vehicle2_location.location.x += 10.0
        vehicle2_location.location.z += 2
        vehicle3_transform = carla.Transform(vehicle2_location.location, ego_rotation.rotation)
        print(vehicle3_transform)
        vehicle3_vehicle = world.spawn_actor(vehicle3_bp, vehicle3_transform)
        print('\nvehicle3 is spawned')
        ...
        time.sleep(5.0)
        for a in range (100):
            vehicle3_vehicle.apply_control(carla.VehicleControl(brake = 1.0))
            vehicle2_vehicle.apply_control(carla.VehicleControl(brake = 1.0))
            ...
            while True:
                ...
                while dist_check:
                    ...
                    if dist_check is not True: 
                        ego_vehicle.apply_control(carla.VehicleControl(throttle = 0.6)) 


        def loop_b():
            while True:
                dist2_check = is_within_distance_ahead(vehicle3_vehicle.get_transform(), vehicle2_vehicle.get_transform(), ACC_DIST)
                vehicle3_vehicle.apply_control(carla.VehicleControl(throttle = 0.5))
                while dist2_check:
                    vehicle2_vehicle.apply_control(carla.VehicleControl(throttle = 0.3))
                    time.sleep(2.0)
                    dist2_check = is_within_distance_ahead(vehicle3_vehicle.get_transform(), vehicle2_vehicle.get_transform(), ACC_DIST)
                    if dist2_check is not True: 
