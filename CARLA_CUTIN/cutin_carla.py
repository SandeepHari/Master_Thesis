...
def main():
    ...
    try:
        ...
        ego_loc = carla.Location(13.568643, 1.1460284, 1.322175)
        ...
        vehicle3_bp = world.get_blueprint_library().find('vehicle.lincoln.mkz2017')    
        ...
        veh_loc = carla.Location(13.568643, -1.960284, 1.322175)
        ...
        def loop_a():
            ...
            while True:
                ...
                if not cond:
                    vehicle3_vehicle.apply_control(carla.VehicleControl(throttle = 0.2))
                    time.sleep(1.0)
                    vehicle3_vehicle.apply_control(carla.VehicleControl(throttle = 0.2, steer = 0.17))
                    time.sleep(3.5)
                    print("First steer")
                    vehicle3_vehicle.apply_control(carla.VehicleControl(throttle = 0.2, steer = -0.28))
                    time.sleep(3.5)
                    print("Second steer")
                
                
                    
                    vehicle3_vehicle.apply_control(carla.VehicleControl(throttle = 0.2, steer = 0.0))
                    time.sleep(1.0)
                    #v3_pos = vehicle3_vehicle.get_transform()
            ...
            while dist_check: 
                ...
                #time.sleep(2.0)
                dist_control = is_within_distance_ahead(vehicle2_vehicle.get_transform(), ego_vehicle.get_transform(), 13.0)
                if dist_control:
                    ego_vehicle.apply_control(carla.VehicleControl(throttle = 0.1))
                    #dist_control = is_within_distance_ahead(vehicle2_vehicle.get_transform(), ego_vehicle.get_transform(), 7.0)
                dist_check = is_within_distance_ahead(vehicle3_vehicle.get_transform(), vehicle2_vehicle.get_transform(), 13.0)
        ...
        def loop_b() :
            ...
            while True:
                actors = world.get_actors()
                global ego_velocity
                ego_velocity.append(get_speed(ego_vehicle))
                #print(ego_velocity[count])
                global lead_velocity
                lead_velocity.append(get_speed(vehicle2_vehicle))
                global cutin_velocity
                cutin_velocity.append(get_speed(vehicle3_vehicle))
                global iteration
        ...
        global lead_velocity
        global cutin_velocity
        ego_array = np.array(ego_velocity)
        lead_array = np.array(lead_velocity)
        cutin_array = np.array(cutin_velocity)
        fig, axs = plt.subplots(3)
        fig.suptitle('velocity profile of vehicles')
        axs[0].plot(iteration, cutin_velocity)
        axs[0].set_title("Cut-in Vehicle")
        axs[1].plot(iteration, lead_velocity)
        axs[1].set_title("lead Vehicle")
        axs[2].plot(iteration, ego_velocity)
